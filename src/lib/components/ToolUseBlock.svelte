...
<CollapsibleSection accentClass="border-l-primary/50" bodyClass="space-y-2">
	{#snippet header()}
		<span
		...
		{#if tool.result?.isError}
			<span class="text-error bg-error/10 ml-auto rounded-full px-2 py-0.5 text-[10px]">error</span>
		{/if}
	{/snippet}
