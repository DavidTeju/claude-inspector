...
{:else if !expanded}
	...
	<button
		...
		class="text-muted-foreground hover:text-foreground/80 border-border/60 hover:border-border flex w-full flex-wrap items-center gap-x-2 gap-y-1 overflow-hidden rounded-md border border-dashed px-3 py-1.5 text-left transition-colors"
	>
		<span class="text-[11px]">{tools.length} tool calls</span>
		<span class="text-muted-foreground/50 text-[10px]">&mdash;</span>
		...
		{/each}
		{#if errorCount > 0}
			<span class="text-destructive ml-auto text-[10px]">
...
{:else}
	...
	<div transition:slide={{ duration: 200, easing: cubicOut }}>
		...
		<button
			...
			class="text-muted-foreground hover:text-foreground/80 mt-2 w-full py-1 text-center text-[10px] transition-colors"
		>
