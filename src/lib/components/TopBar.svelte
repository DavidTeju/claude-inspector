...
<header
	class="border-base-content/10 bg-base-100/88 flex items-center gap-4 border-b px-4 py-2.5 backdrop-blur-md lg:px-6"
>
	{#if !sidebarOpen}
		<button onclick={onToggleSidebar} class="btn btn-ghost btn-circle" aria-label="Open sidebar">
	...
	{#if breadcrumbs.length > 0}
		...
		<nav class="text-base-content/50 flex min-w-0 items-center gap-2 text-[0.9rem]">
			{#each breadcrumbs as crumb, i (crumb.path)}
				...
				{:else}
					<a
						...
						class="hover:text-base-content max-w-[14rem] truncate transition-colors"
					>
						{crumb.label}
					</a>
				{/if}
	...
	<div class="ml-auto flex items-center gap-3">
		...
		<a href={resolve('/')} class="btn btn-ghost btn-circle" aria-label="Search sessions">
			<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path
					stroke-linecap="round"
					stroke-linejoin="round"
