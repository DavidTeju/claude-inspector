<script lang="ts">
	...
	let breadcrumbs = $derived.by(() => {
		...
		const parts: Array<{ label: string; href: string }> = [{ label: 'Home', href: '/' }];

		if (url.startsWith('/projects/')) {
			const projectId = page.params?.projectId;
			if (projectId) {
				parts.push({ label: decodeProjectName(projectId), href: `/projects/${projectId}` });
			}
		}
		...
		if (url.startsWith('/session/')) {
			...
			if (sessionId) {
				parts.push({
					...
					href: `/session/${projectId}/${sessionId}`
				});
			}
		}
	...
	function decodeProjectName(id: string): string {
		let name = id.startsWith('-') ? id.slice(1) : id;
...
>
	...
	<form onsubmit={handleSearch} class="ml-auto flex items-center">
		<div class="relative">
			<svg
