	...
	<div class="relative">
		...
		{#if showCommandList}
			...
			>
				{#each filteredCommands as cmd, i (cmd.name)}
					...
					>
						<span class="flex items-baseline gap-2 text-xs">
							...
							{#if cmd.argumentHint}
								<span class="text-base-content/40 shrink-0">{cmd.argumentHint}</span>
							{/if}
						</span>
				...
				<div
					class="border-base-content/15 text-base-content/30 flex gap-2 border-t px-3 py-1.5 text-[10px]"
				>
					<span><kbd class="kbd kbd-xs">↑↓</kbd> navigate</span>
					<span class="text-base-content/15">&middot;</span>
					<span><kbd class="kbd kbd-xs">Tab</kbd> accept</span>
					<span class="text-base-content/15">&middot;</span>
					<span><kbd class="kbd kbd-xs">Esc</kbd> dismiss</span>
				</div>
			</div>
		{/if}
