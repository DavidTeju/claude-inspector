...
{#if typeof tool.input.command === 'string'}
	<div class="overflow-hidden rounded-md">
		<!-- Terminal header bar -->
		<div class="bg-neutral flex items-center gap-2 px-3 py-1.5">
...
{#if tool.result !== undefined}
	<div class="relative">
		{#if isError}
			<span
				class="text-error bg-error/15 absolute top-2 right-2 rounded-full px-2 py-0.5 font-mono text-[9px] font-medium"
				>exit 1</span
			>
		{/if}
		<pre
