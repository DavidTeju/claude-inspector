...
{#if typeof tool.input.command === 'string'}
	<div class="overflow-hidden rounded-md">
		<!-- Terminal header bar -->
		<div class="bg-muted flex items-center gap-2 px-3 py-1.5">
		...
		<!-- Command -->
		<pre
			class="bg-primary/8 border-primary/15 text-primary max-h-32 overflow-auto border-b px-3 py-2 font-mono text-[11px] leading-relaxed font-bold break-words whitespace-pre-wrap"><span
...
{#if tool.result !== undefined}
	<div class="relative">
		{#if isError}
			<span
				class="text-destructive bg-destructive/15 absolute top-2 right-2 rounded-full px-2 py-0.5 font-mono text-[9px] font-medium"
				>exit 1</span
			>
		{/if}
		<pre
			class="bg-background max-h-96 overflow-auto rounded-md p-3 text-[11px] {isError
				? 'text-destructive'
				: 'text-foreground/80'} font-mono leading-relaxed break-words whitespace-pre-wrap">{resultText}</pre>
	</div>
{/if}
