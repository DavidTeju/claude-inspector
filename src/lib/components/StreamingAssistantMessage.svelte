...
<div class="max-w-[95%]">
	<div class="border-l-accent-400/30 bg-card/70 rounded-xl rounded-bl-md border-l-2 px-4 py-3">
		<!-- Header -->
		<div class="mb-2 flex items-center gap-2">
			<span class="text-primary text-[10px] font-semibold tracking-wider uppercase">Claude</span>
			{#if model}
				<span class="text-muted-foreground text-[9px]">{model}</span>
			{/if}
			...
			{#if splitContent.complete || splitContent.trailing || text?.trim()}
				<div>
					...
					{#if splitContent.trailing || (!splitContent.complete && text?.trim())}
						<div
							class="text-foreground/80 text-sm leading-relaxed {splitContent.complete
								? 'mt-1'
								: ''}"
						>
