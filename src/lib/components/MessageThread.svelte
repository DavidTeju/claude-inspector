...
>
	<div class="space-y-6 p-6">
		...
		{:else if !session}
			<div class="text-muted-foreground py-12 text-center text-sm">No messages in this session.</div>
		{/if}

		<!-- Streaming assistant message -->
		{#if session?.streamingUuid}
			<div class="border-border/50 relative border-l pl-6">
				<div
	...
	{#if !isNearBottom}
		<button
			...
			class="border-border bg-muted text-foreground/80 hover:bg-muted absolute bottom-4 left-1/2 -translate-x-1/2 cursor-pointer rounded-full border px-3 py-1.5 text-[10px] shadow-lg transition-colors"
		>
			Jump to bottom
		</button>
	{/if}
