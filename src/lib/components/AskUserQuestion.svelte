...
<div class="border-l-secondary bg-secondary/10 rounded-xl border-l-2 px-4 py-3">
	{#each request.questions as question, qi (qi)}
		<div class={qi > 0 ? 'border-base-content/5 mt-4 border-t pt-4' : ''}>
			{#if question.header}
			...
			<div class="space-y-1" role={question.multiSelect ? 'group' : 'radiogroup'}>
				{#each question.options as option (option.label)}
					...
					<button
						...
						class="hover:bg-neutral/30 flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors {selected
							? 'border-secondary/30 bg-neutral/50 border'
				...
				{#if question.options.length > 2}
					...
					<button
						...
						class="hover:bg-neutral/30 flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors {otherSelected
							? 'border-secondary/30 bg-neutral/50 border'
							: 'border border-transparent'}"
					>
						<!-- Radio / Checkbox indicator -->
	...
	<div class="mt-3">
		<button
			...
			class="bg-secondary text-base-100 rounded-lg px-4 py-2 text-[11px] font-semibold transition-colors {hasAnyAnswer
