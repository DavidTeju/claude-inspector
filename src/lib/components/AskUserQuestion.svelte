...
<div class="border-l-user bg-user-muted/20 rounded-xl border-l-2 px-4 py-3">
	{#each request.questions as question, qi (qi)}
		<div class={qi > 0 ? 'border-surface-800/50 mt-4 border-t pt-4' : ''}>
			{#if question.header}
				<div class="text-foreground mb-1 text-[11px] font-semibold">{question.header}</div>
			{/if}
			<div class="text-foreground/80 mb-2 text-sm">{question.question}</div>
			...
			<div class="space-y-1" role={question.multiSelect ? 'group' : 'radiogroup'}>
				{#each question.options as option (option.label)}
					...
					<button
						...
						class="hover:bg-accent/30 flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors {selected
							? 'border-user-400/30 bg-accent/50 border'
					...
					>
						...
						>
							{#if selected}
								<svg
									class="text-primary-foreground h-2.5 w-2.5"
									fill="none"
									viewBox="0 0 24 24"
									stroke="currentColor"
									stroke-width="3"
						...
						<div>
							...
							{#if option.description}
								<span class="text-muted-foreground ml-1 text-[10px]">{option.description}</span>
							{/if}
						</div>
					</button>
				{/each}
				...
				{#if question.options.length > 2}
					...
					<button
						...
						class="hover:bg-accent/30 flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors {otherSelected
							? 'border-user-400/30 bg-accent/50 border'
							: 'border border-transparent'}"
					>
						<!-- Radio / Checkbox indicator -->
						...
						<input
							...
							class="min-w-0 flex-1 bg-transparent text-sm focus:outline-none {otherSelected
								? 'text-text-100 placeholder-muted-foreground cursor-text'
	...
	<div class="mt-3">
		<button
			...
			class="bg-user-500 text-primary-foreground rounded-lg px-4 py-2 text-[11px] font-semibold transition-colors {hasAnyAnswer
				? 'hover:bg-user cursor-pointer'
